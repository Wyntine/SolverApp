...
export class Table {
  ...
  public getCells(): Cell[] {
    return this.data.flatMap((item) => item).filter((item) => item.isCell());
  }
  ...
  public logCellsCompact(): void {
    ...
    for (const group of groups) {
      console.log(
        "-".repeat(5) +
          (group.at(0)?.getGroupId(true) ?? "No ID") +
          "-".repeat(5),
      );
      console.log(
        group
          .map((cell) => {
            ...
            return `x: ${x.toString()}, y: ${y.toString()} | value: ${(
              value ?? "None"
            ).toString()} | pValues: ${possibleValues.join(", ")}`;
          })
          .join("\n"),
      );
    }
            ...
            (column) =>
              column instanceof Point
                ...
                ? isOdd(lineIndex)
                  ...
                  : "│" // dim gray
                : (() => {
                    const check = column.hasValue()
                      ? column.getValue() ?? "None"
                      : " ";
                    const value = check.toString();
